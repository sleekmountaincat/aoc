...
hands.sort((x, y) => {
    ...
    for (let i = 0; i < x.hand.length; i++) {
        ...
        if (tieWinner === 100 && rankValues.get(x.hand[i])! > rankValues.get(y.hand[i])!) tieWinner = -1 // x comes first
        else if (tieWinner === 100 && rankValues.get(x.hand[i])! < rankValues.get(y.hand[i])!) tieWinner = 1 // y comes first
    }

    if (tieWinner === 100) tieWinner = 0
