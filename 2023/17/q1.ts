...
const costs = fs.readFileSync('2023/17/inputtest.txt')
    .toString()
    .split("\n")
...
const moveThatDangLava = (endX: number, endY: number): Node[] => {
    ...
    while (!heap.isEmpty()) {
        ...
        // if we have found the end, rejoice
        ...
        // if we have been going same direction for 3 moves, must go 90 degrees
        if (cur.sinceTurn! >= 3) {
            ...
            // otherwise just cant go same dir
            if (cur.dir == "R") {
                directions.push(R)
                directions.push(U)
                directions.push(D)
