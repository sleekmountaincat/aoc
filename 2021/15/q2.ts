...
const graphTile: number[][] = fs.readFileSync('2021/15/input.txt')
    .toString()
    .split("\n")
...
const graph: number[][] = []
const scale = 5
const size = graphTile.length

for(let i = 0; i < (size*scale); i++) {
    graph.push([])
    for(let j = 0; j < (size*scale); j++) {
        graph[i][j] = ((graphTile[i % size][j % size] + Math.floor(i/size) + Math.floor(j/size))  % 9) || 9
    }
}
